...
#variable "namecheap_extra_domains_for_cloudflare_ns_forward" {
#  type        = list(string)
#  description = "Extra domains to forward to Cloudflare nameservers"
#  default     = []
#  # validate its valid domains
#  validation {
#    condition     = alltrue([for domain in var.namecheap_extra_domains_for_cloudflare_ns_forward : can(regex("^[a-zA-Z0-9-]+\\.[a-zA-Z]{2,}$", domain))])
#    error_message = "The namecheap_extra_domains_for_cloudflare_ns_forward must be a list of valid domain names."
#  }
#}

#locals {
#  namecheap_domains_for_cloudflare_ns_forward = flatten([
#    var.namecheap_extra_domains_for_cloudflare_ns_forward,
#    [
#      "d3strukt0r.me",
#      "manuele-vaccari.ch",
#    ],
...
#resource "namecheap_domain_records" "namecheap_to_cloudflare" {
#  for_each = toset(local.namecheap_domains_for_cloudflare_ns_forward)
#  domain   = each.key
#  # note: namecheap now requires that the 'custom' nameservers are first supplied through the ui
#  nameservers = [
